...
{    
  ...
  public function changeHead(Request $request){      
    // init 
    $src = $request->input('avatar_src');
    ...
    if($response['result']){
      $head_pic = '/'.$response['result'];
      if($user){
        $res = Users::where('id',$user['id'])->update(['avatar'=>$head_pic]);
        if($res){
          return $response;
        }
      }
  ...
  private function setFile($file) {
    ...
    if ($errorCode === UPLOAD_ERR_OK) {
      ...
      if ($type) {
        ...
        $src = 'user_head_pic/' . date('YmdHis') . '.original' . $extension;
  ...
  private function setDst() {
    $this -> dst = 'user_head_pic/uhead_' . $this->user_id . '_ser.png';
  }
